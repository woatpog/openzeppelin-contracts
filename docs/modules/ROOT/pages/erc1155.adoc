...
**TIP:** If you'd like to put all item information on-chain, you can extend ERC-721 to do so (though it will be rather costly) by providing a [`Base64`](xref:utilities.adoc#base64) Data URI with the JSON schema encoded. You could also leverage IPFS to store the URI information, but these techniques are out of the scope of this overview guide.

## Sending Tokens to Contracts

A key difference when using [`safeTransferFrom`](xref:api:token/ERC1155.adoc#IERC1155-safeTransferFrom-address-address-uint256-uint256-bytes-) is that token transfers to other contracts may revert with the following custom error:
...
In order for our contract to receive ERC-1155 tokens, we can inherit from the convenience contract [`ERC1155Holder`](xref:api:token/ERC1155.adoc#ERC1155Holder) which handles the registering for us. However, we need to remember to implement functionality to allow tokens to be transferred out of our contract:

```solidity
include::api:example$token/ERC1155/MyERC1155HolderContract.sol[]
```

We can also implement more complex scenarios using the [`onERC1155Received`](xref:api:token/ERC1155.adoc#IERC1155Receiver-onERC1155Received-address-address-uint256-uint256-bytes-) and [`onERC1155BatchReceived`](xref:api:token/ERC1155.adoc#IERC1155Receiver-onERC1155BatchReceived-address-address-uint256---uint256---bytes-) functions.
